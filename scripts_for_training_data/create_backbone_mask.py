...
This tool generates masks for backbone with the following labels:
...
class BackboneMask:
    """
    Generate masks for backbone with the following labels:
    0: Non-structural elements (background)
    1: Neighboring voxels (26-neighbor connectivity)
    2: Non-backbone atoms
    3: Backbone atoms (N, CA, C, O)
                    ...
                    for residue in chain:
                        for atom in residue:
                            ...
                            if atom.get_name() in ['N', 'CA', 'C', 'O']:
                                mask[pos] = 3  # Backbone atoms
                            else:
                                mask[pos] = 2  # Other atoms
                            
                            assigned_positions[pos] = mask[pos]
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate Backbone masks",
    ...
    for i, directory in enumerate(BASE_DIR):
        ...
        save_path = f"{output_directory}/backbone_mask.mrc"
        
        try:
            mask_gen = BackboneMask(normalized_map)
            mask = mask_gen.generate_mask(pdb_file)
            mask_gen.save_mask(mask, save_path)
            print(f"Generated Backbone Mask for EMD ID: {emd_id} | Completed {i + 1} density maps ...")
        except Exception as e:
