#!/usr/bin/env python3
"""
CA Mask Grid Generator

Author: Rajan Gyawali
Date: June 01, 2025

This tool creates grids from carbon alpha mask files and saves each grid with its indices.
"""
...
def create_and_save_grids(mrc_file, output_dir, grid_size=48, padding=8):
    """
    Create grids from carbon alpha  mask file and save each grid with its indices
...
def main():
    parser = argparse.ArgumentParser(
        description="Create grids from carbon alpha mask files",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
        Author: Rajan Gyawali
        Date: June 01, 2025

        This tool creates grids from carbon alpha mask files with the following features:
    ...
    parser.add_argument("--base-dir", default="Training_Dataset/Processed_Data",
                       help="Base directory containing processed files with CA masks")
    parser.add_argument("--output-dir", default="Training_Dataset/Grids/CA_masks",
                       help="Output directory for CA mask grid files")
    ...
    for i, directory in enumerate(BASE_DIR):
        ...
        carbon_alpha_mask = f"{directory}/carbon_alpha_mask.mrc"
        output_directory = f"{OUTPUT_DIR}/{emd_id}"
        os.makedirs(output_directory, exist_ok=True)
        
        try:
            grid_count = create_and_save_grids(carbon_alpha_mask, output_directory,
                                             args.grid_size, args.padding)
            print(f"Created {grid_count} number of CA mask grids for EMD ID: {emd_id} | Completed {i + 1} masks")
        except Exception as e:
            print(f"CA mask grid creation failed for EMD ID: {emd_id} - Error: {str(e)}")
